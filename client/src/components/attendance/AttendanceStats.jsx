...
const AttendanceStats = ({history}) => {
    ...
    const completed = history.filter((h)=> h.punchOut?.time).length;
    const totalHours = history.reduce((sum, h)=> sum + (Number(h.workingHours) || 0), 0).toFixed(1);

    const stats = [
        {label: "Days Present", value: totalPresent, icon: CalendarIcon},
        {label: "Completed Days", value: completed, icon: AlertCircleIcon},
        {label: "Total Hours", value: totalHours, icon: ClockIcon},
    ]
  return (
