...
import {BellIcon, BriefcaseBusinessIcon, CalendarCheckIcon, CalendarIcon, ChevronRightIcon, ClipboardListIcon, FilePenLineIcon, FileTextIcon, LayoutGridIcon, Loader2, LogOutIcon, MapPinnedIcon, MenuIcon, SettingsIcon, UserIcon, XIcon} from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import api from '../api/axios'
import { employeeName, unwrap } from '../api/helpers'
...
const Sidebar = () => {
    ...
    useEffect(()=>{
        api.get("/employees/me/profile").then((response)=> {
            const employee = unwrap(response).employee;
            setUserName(employeeName(employee));
        }).catch(()=> setUserName(employeeName(user)))
    },[user])
    ...
    const navItems = [
        ...
        {name: "Corrections", href: "/corrections", icon: FilePenLineIcon},
        {name: "Leave", href: "/leave", icon: FileTextIcon},
        {name: "Holidays", href: "/holidays", icon: CalendarCheckIcon},
        {name: "Announcements", href: "/announcements", icon: BellIcon},
        {name: "Clients", href: "/clients", icon: BriefcaseBusinessIcon},
        {name: "Activities", href: "/activities", icon: ClipboardListIcon},
        {name: "Visits", href: "/visits", icon: MapPinnedIcon},
        ...
        <>
            ...
            <div className='px-5 pt-6 pb-5 border-b border-white/6'>
                <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                            <p className='font-semibold text-[13px] text-white tracking-wide'>SharpKode</p>
                            <p className='text-[11px] text-slate-500 font-medium'>Workforce</p>
                        </div>
                    </div>
            ...
            <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                {loading ? (
                    ...
                    return (
                        <Link onClick={()=> setMobileOpen(false)} key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
  ...
  return (
    <>
        ...
        {/* Sidebar - desktop */}
        <aside className='hidden lg:flex flex-col h-full w-68 bg-linear-to-b from-slate-950 via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/8 shadow-[18px_0_55px_rgba(15,23,42,0.08)]'>
            {sidebarContent}
        </aside>
        ...
        <MotionAside
          ...
          className='lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-950 via-slate-900 to-slate-950 text-white z-50 flex flex-col shadow-2xl'
        >
            {sidebarContent}
        </MotionAside>
    </>
